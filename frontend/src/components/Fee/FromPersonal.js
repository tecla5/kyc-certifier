...
export default class FromPersonal extends Component {
  ...
  handleWhoChange = async (_, { value }) => {
    ...
    try {
      const localAccounts = await appStore.getAccounts();
      const knownAccount = EthAccounts[value];
      const valid = isValidAddress(value) && !knownAccount;

      if (localAccounts.find(({ address }) => address.toLowerCase() === value.toLowerCase())) {
        return this.setState({
          loading: false,
          valid: false,
          reason: 'It looks like you are trying to certify the address you have sent the fee to. Please provide a valid Ethereum address you own.'
        });
      }
