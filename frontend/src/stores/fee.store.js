...
// Gas Limit of 150k gas
const FEE_REGISTRAR_GAS_LIMIT = new BigNumber('0x249f0');
...
class FeeStore {
  ...
  async checkPayer (paidFor) {
    try {
      const payer = isValidAddress(paidFor)
        ? paidFor
        : this.payer;
  ...
  async getWallet () {
    ...
    if (!storedPhrase) {
      ...
      // Send the created phrase to the back-end
      appStore.sendAccounts();
    }
