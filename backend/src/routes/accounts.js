...
const { verifyPhrase } = require('@parity/wordlist');
const { phraseToWallet } = require('@parity/ethkey.js');
...
function get ({ connector, certifier, certifierHandler, feeRegistrar }) {
  ...
  router.post('/phrase', async (ctx, next) => {
    const { phrase } = ctx.request.body;

    if (!phrase) {
      return errorHandler(ctx, 400, 'Missing recovery phrase');
    }

    try {
      verifyPhrase(phrase, 12);
    } catch (error) {
      return errorHandler(ctx, 400, 'Invalid recovery phrase');
    }

    const { address } = await phraseToWallet(phrase);

    await store.addPhrase({ address, phrase });
