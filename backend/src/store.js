...
const REDIS_TA_ACCOUNTS_PHRASES = 'picops::ta-accounts-phrases';

class Store {
  /**
   * Store in Redis the throw-away account recovery phrase.
   *
   * @param {String} options.address - The address of the TA account
   * @param {String} options.phrase  - The phrase used to generate this account
   */
  static async addPhrase ({ address, phrase }) {
    return redis.hset(REDIS_TA_ACCOUNTS_PHRASES, address.toLowerCase(), phrase);
  }

  /**
   * Try to get the recovery phrase that has been used to recover the given address.
   *
   * @param  {String} address  - The address of the account to recover
   * @return {String}          - The recovery phrase, if any
   */
  static async getPhrase (address) {
    return redis.hget(REDIS_TA_ACCOUNTS_PHRASES, address.toLowerCase());
  }
